...
public class TElementoAB<T> implements IElementoAB<T>
{
    ...
    {
        return (etiqueta.toString()); //Todos los ELEMENTOS tienen etiqueta.
    }
    
    @Override
    public void inOrdenListaDatoYetiqueta(Lista<T> unaLista)
    {
        if (hijoIzq != null)
        {
            hijoIzq.inOrdenListaDatoYetiqueta(unaLista);
        }
        Nodo<T> unNodo = new Nodo<>(this.getEtiqueta(),this.getDatos());
        unaLista.insertar(unNodo);
        if (hijoDer != null)
        {
            hijoDer.inOrdenListaDatoYetiqueta(unaLista);
        }
    }
    ...
    public IElementoAB<T> eliminar(Comparable unaEtiqueta)
    {
        if (unaEtiqueta.compareTo(this.getEtiqueta()) < 0)
        {
            ...
            {
                this.hijoIzq = hijoIzq.eliminar(unaEtiqueta); //Cuando encuentra el valor 
            }
            return this;
        }
